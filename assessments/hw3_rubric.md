...
| Level | Points | Criteria |
|---|---|---|
| **Excellent** | 15 | `NOTE.md` is five to nine sentences a reader without SQL can act on: the metric (what is divided by what, for which countries and years, in which unit); the EU-27 figures for 2010 and 2024 and the 2024 leaders on both measures, matching gold; what was left out and how many; the assumptions a reader could miss; the reconciliation in plain words; OWID against Eurostat in a sentence or two (the median residual, how much the two sources' documentation explains, and that the rest is unexplained); and **what the data cannot answer, stated as a claim about the data** ("this cannot tell you X, because the data has no Y"). |
| **Satisfactory** | 10 | The note defines its metric and is about the result, and one or two of the other elements are missing or wrong. A "cannot answer" that is a hedge ("more research is needed", "data quality may vary") rather than a claim about the data counts as one element wrong; so does an OWID–Eurostat sentence that is missing or explains nothing. |
